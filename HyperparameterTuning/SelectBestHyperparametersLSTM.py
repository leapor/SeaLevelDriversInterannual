...
for s in stat:
	...
	fig, ax = plt.subplots(len(activation), len(sequence_len), figsize = [10, 12], \
		sharex = True, sharey = True)
	plt.subplots_adjust(left=0.1, bottom=0.07, right=0.95, top=0.92, hspace=0.0, wspace=0.0)
	...
	for ai, a in zip(range(len(activation)), activation):
		for li, l in zip(range(len(sequence_len)), sequence_len):
			# plot data
			x = hyp['ExpVar'].loc[{dim['s']:s, dim['a']:a, dim['n']:l}].values
			x = np.transpose(x)
			...
			if (a == best_med.loc[s,dim['a']]) and (l == best_med.loc[s,dim['n']]):
				ind = np.where((units == best_med.loc[s,dim['u']]))[0][0]
				change_boxplot_colors(bp, ind, *col1)
			
			# based best model based on mean
			if (a == best_mean.loc[s,dim['a']]) and (l == best_mean.loc[s,dim['n']]):
			...
			if (ai==0): ax[ai,li].set_title(dim['n']+': '+str(l), fontsize = 20)
			if (li==len(sequence_len)-1): ax[ai,li].text(1.02, 0.5, a, fontsize = 20, \
	...
	fig.text(0.5, 0.99, 'Tuning LSTM for '+statname[s]+' ('+str(s)+')', fontsize = 24, \
		ha = 'center', va = 'top')
