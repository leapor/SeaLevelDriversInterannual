...
# Extracting ERA5 variables that are integrated from the box around each station's location.
...
from Functions import get_config, prepare_map, select_subset_ERA5, extract_ERA5_integ, \
...
box_size = 2  # how many degrees on each side of the station to extract data from

var = config['drivers']['integ']
...
for i in range(len(var)):
	x = extract_ERA5_integ(infile.replace('[var]', var[i]), var[i], coords, box_size, [y1, y2])
...
# ------------------------------------------------------------------------------------------------
# --------- Plotting --------- (same as in ERA5localPoint)
# ------------------------------------------------------------------------------------------------
...
for i in range(len(var)):
	...
	for ti in range(len(timesteps)):
		...
		prepare_map(ax[i], lonlim = [lon1, lon2], latlim = [lat1, lat2])
		...
		ax[ti].scatter(data2.lon, data2.lat, c=data2.loc[:, timesteps[ti]].values, \
			...
			edgecolors = 'k', s = 200)
		ax[ti].add_feature(cfeature.LAND, edgecolor = 'grey', facecolor = 'none', \
			zorder = 100)
