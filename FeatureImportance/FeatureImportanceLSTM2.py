...
# importance.
# 2nd LSTM experiment for the whole region.
...
from Functions import get_config, split_data, create_sequence, RRMSE, relative_explained_variance, \
	corr2
...
for s, si in zip(stat, range(nstat)):
	...
	if (si == 0):
		...
		data_vars = {met[0]: ((dim['s'], dim['m'], dim['f']), \
			np.full((len(stat),N,len(features)), np.nan)), \
			met[1]: ((dim['s'], dim['m'], dim['f']), \
			np.full((len(stat),N,len(features)), np.nan)), \
			met[2]: ((dim['s'], dim['m'], dim['f']), \
			np.full((len(stat),N,len(features)), np.nan))}
	...
	for i in range(N):		
		...
		metrics[met[1]].loc[{dim['s']:s, dim['m']:i, dim['f']:'all'}] = \
			RRMSE(ytrue, ypred)
		metrics[met[2]].loc[{dim['s']:s, dim['m']:i, dim['f']:'all'}] = \
			corr2(ytrue, ypred)
		...
		for f, fi in zip(features[1:], range(len(features)-1)):
			...
			metrics[met[1]].loc[{dim['s']:s, dim['m']:i, dim['f']:f}] = \
				RRMSE(ytrue, ypred)
			metrics[met[2]].loc[{dim['s']:s, dim['m']:i, dim['f']:f}] = \
				corr2(ytrue, ypred)
