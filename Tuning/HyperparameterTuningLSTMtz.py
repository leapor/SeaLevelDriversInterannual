#!/usr/bin/env python3
# Finding the best hyperparameters for the LSTM networks in the Baltic-North Sea
# transition zone.
# Tests for number of units per layer (same in each layer), activation function
# (same in each layer) and sequence length.
...
listfile = config['dirs']['data'] + config['dirs']['ana'] + 'TransitionZoneStationsSelected.csv'

indir = config['dirs']['data'] + config['dirs']['pro']
infile = 'data_station_{stat:n}_train.csv'

outdir = config['dirs']['data'] + config['dirs']['tune']
outfile = 'LSTM_tuning_TZ.nc'

sequence_len = [13] #[2, 3, 4, 5, 6, 7, 8, 13]
activation = ['relu', 'sigmoid', 'tanh']
units = [2, 3, 5, 10, 20, 30, 50, 100, 150, 200, 300]
...
stat = pd.read_csv(listfile, index_col = 0)
ntot = stat.shape[0]
