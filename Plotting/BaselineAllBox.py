...
nr, nc = 12, 4
figsize = [26, 32]
subplots_adjust=dict(bottom=0.04,top=0.98,left=0.06,right=0.99,wspace=0.13,hspace=0.26)
w = 0.4
alpha = 0.3
lw1, lw2, lwf = 1, 2, 2
col = {'NN': 'maroon', 'LR': 'teal', 'best': 'lightgrey'}

fs0, fs1, fs2, fs3 = 32, 26, 21, 16 # font size for xy labels, legend, subtitles, tick labels
dy = 0.2
tx, ty = 0.01, 1+dy/2
boxpar = dict(alpha=0.4, edgecolor='k', lw=1)
txtpar = dict(ha='left', va='center', fontsize=fs2, stretch='condensed', clip_on=False)
# ------------------------------------------------------------------------------
...
for s, a in zip(statord.flat, ax.flat):
    ...
    # station name and number (shift the ylim to accomodate it); color represents region
    a.add_patch(plt.Rectangle((0,1), 1, dy, transform=a.transAxes, facecolor=colstat[s], **boxpar, clip_on=False))
    txt = '{:s} ({:d}): {:s}'.format(name[s], s, bestmod)
    a.text(tx,ty, txt, **txtpar, transform=a.transAxes, zorder=203)
    ...
    # frame around ax+title box
    a.add_patch(plt.Rectangle((0,0), 1, 1+dy, transform=a.transAxes, fill=False, edgecolor='k', lw=1.5, clip_on=False, \
        zorder=100))
...
for i, ri in zip(range(len(reg)), reg.values()):
    dummy[i] = plt.scatter([],[], s=4000, marker=rect, facecolor=ri['col'], label=ri['name'], **boxpar)
